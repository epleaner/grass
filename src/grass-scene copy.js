...
export class GrassScene {
  ...
  createGrass() {
    ...
    const opacities = [];
    const scales = [];
    const bendFactors = []; // New: how much each segment bends
    const segmentsPerBlade = 4; // Number of segments per grass blade
    const bladeHeight = 0.6;

    // Create simplex noise for varied bending
    const simplex = new SimplexNoise();
    ...
    for (let i = 0; i < grassCount; i++) {
      ...
      for (let j = 0; j < segmentsPerBlade; j++) {
        ...
        // Each point needs its own offset and bend factor
        const noiseVal = simplex.noise3d(x * 0.1, y * 0.1, z * 0.1);
        const bendFactor = Math.pow(t, 2) * (0.8 + noiseVal * 0.4); // Quadratic falloff for natural bending
    ...
    this.grassMaterial = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          ...
          // Create varied wind effect using noise
          float timeOffset = time * 1.5 + offset;
          vec2 noiseCoord = vec2(transformed.x * 0.05 + timeOffset, transformed.z * 0.05);
          float noise = snoise(noiseCoord) * windStrength;
          
          // Add second layer of noise for more natural movement
          noiseCoord = vec2(transformed.x * 0.1 - timeOffset * 0.5, transformed.z * 0.1);
          noise += snoise(noiseCoord) * 0.5 * windStrength;

          // Apply bending based on height and noise
          transformed.x += noise * bendFactor * scale;
          transformed.z += noise * 0.5 * bendFactor * scale;
          
          // Slight vertical compression with strong wind
          transformed.y -= abs(noise) * 0.1 * bendFactor * scale;
          
          vOpacity = opacity * (0.4 + 0.6 * (1.0 - abs(noise) * 0.5));
  ...
  animate = () => {
    ...
    this.windStrength = 0.5 + Math.sin(Date.now() * 0.001) * 0.5;
    this.grassMaterial.uniforms.windStrength.value = this.windStrength;
    this.grassMaterial.uniforms.time.value += 0.015;
